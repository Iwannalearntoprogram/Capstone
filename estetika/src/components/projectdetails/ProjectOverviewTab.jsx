...
function SectionPanel({ eyebrow, title, description, children, className = "" }) {
  return (
    <section className={`rounded-[24px] border border-black/5 bg-white p-5 sm:p-6 ${className}`}>
...
function DetailItem({ label, value, className = "" }) {
  return (
    <div className={`rounded-2xl border border-slate-200/80 bg-slate-50/80 px-4 py-4 ${className}`}>
...
export default function ProjectOverviewTab() {
    ...
    return (
      <div className="flex min-h-[280px] items-center justify-center rounded-[24px] border border-dashed border-slate-300 bg-white px-6 text-center">
        <div className="space-y-2">
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-400">Project Overview</p>
          <p className="text-lg font-semibold text-slate-900">No project data available</p>
          <p className="text-sm text-slate-500">Refresh the page or reopen the project from the list.</p>
        </div>
      </div>
    );
  }
  ...
  const projectDetails = [
    ...
    { label: "Timeline", value: formatTimeline(project.startDate, project.endDate) },
    { label: "Location", value: project.projectLocation || "Location not set" },
    {
      label: "Project size",
      value: project.projectSize ? `${project.projectSize} sq ft` : "Size not set",
    },
    { label: "Room type", value: project.roomType || "Room type not set" },
    { label: "Project type", value: project.projectType || "Project type not set" },
  ...
  return (
    <div className="space-y-6">
          ...
          {designers.length > 0 ? (
            <div className="space-y-3">
              {designers.map((designer, index) => (
                <TeamMemberCard key={designer._id || index} designer={designer} />
              ))}
            </div>
          ) : (
            <div className="rounded-[22px] border border-dashed border-slate-300 bg-slate-50/80 px-5 py-10 text-center">
              <p className="text-lg font-semibold text-slate-900">No designers assigned</p>
              <p className="mt-2 text-sm leading-7 text-slate-500">
                Designers will appear here once they are added to the project team.
              </p>
            </div>
          )}
        </SectionPanel>
      ...
      <SectionPanel
        ...
        description="A single material recommendation generated from the project brief, planning priorities, and available catalog context."
      >
        {recommendedMaterial ? (
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(300px,0.9fr)]">
            <div className="space-y-5">
              <div className="flex flex-col gap-4 rounded-[22px] border border-slate-200/80 bg-[#fcfbf8] px-5 py-5 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-2">
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-left sm:min-w-[180px] sm:text-right">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Material price</p>
              ...
              <div className="rounded-[22px] border border-slate-200/80 bg-white px-5 py-5">
                <p className="text-sm leading-8 text-slate-600">
                  {recommendedMaterial.description || "No material description available."}
                </p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <MetricCard label="Style fit" value={formatPercent(recommendationScore?.keywordFit)} />
                <MetricCard label="Budget fit" value={formatPercent(recommendationScore?.budgetFit)} />
                <MetricCard label="Room fit" value={formatPercent(recommendationScore?.roomFit)} />
                <MetricCard label="Overall match" value={formatPercent(recommendationScore?.total)} />
              </div>
            </div>
          </div>
        ) : (
          <div className="rounded-[22px] border border-dashed border-slate-300 bg-slate-50/80 px-5 py-10 text-center">
            <p className="text-lg font-semibold text-slate-900">No material recommendation available yet</p>
            <p className="mt-2 text-sm leading-7 text-slate-500">
              Add richer project details or catalog materials to generate a more relevant recommendation.
            </p>
          </div>
        )}
      </SectionPanel>
          ...
          >
            {project.designRecommendation.imageLink ? (
              <div className="overflow-hidden rounded-[24px] border border-slate-200/80 bg-slate-100">
                <img
                  ...
                  alt={project.designRecommendation.title || "Design recommendation"}
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <DetailItem
                  label="Budget range"
                  value={formatBudgetRange(project.designRecommendation.budgetRange)}
                />
                <DetailItem
                  label="Popularity"
                  value={
              ...
              project.designRecommendation.designPreferences.length > 0 ? (
                <div className="rounded-[22px] border border-slate-200/80 bg-white px-5 py-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {project.designRecommendation.designPreferences.map((preference, index) => (
                      <span
                        key={index}
                        className="rounded-full bg-[#1d3c34]/10 px-3 py-1 text-sm font-medium text-[#1d3c34]"
                      >
                        {preference.replace("-", " ").replace(/\b\w/g, (letter) => letter.toUpperCase())}
                      </span>
                    ))}
                  </div>
                </div>
